...
const StartQuiz = () => {
  ...
  useEffect(() => {
    ...
    if (savedQuizId === quizId && savedQuizData && savedStartTime) {
      ...
      const remaining = (parsedQuiz.timer || 1) * 60 - elapsed;
      ...
      if (isPasswordVerified) fetchQuizData();
    }
  ...
  if (!isPasswordVerified) {
    return (
      <div className="quiz-password-container" id="quiz-password">
        ...
        />
        <button className="quiz-btn" id="start-quiz-btn" onClick={handlePasswordSubmit}>
          Start Quiz
        </button>
      </div>
    );
  ...
  return (
    <div className="start-quiz-container" id="quiz-main">
      ...
      <div className="quiz-controls" id="quiz-controls">
        ...
        <button className="quiz-btn" id="mark-btn" onClick={handleMarkForReview}>
          Mark for Review
        </button>
        <button
          className="quiz-btn"
          id="next-btn"
          onClick={() => setCurrentQuestionIndex((prev) => (prev < quiz.questions.length - 1 ? prev + 1 : prev))}
        >
          Next
        </button>
      </div>
